...
public class HealthBarRenderer {
	...
	public void renderHealthBar(int screenWidth, int screenHeight) {
		...
		for (int i = 9; i >= 0; --i) {
			...
			if (icon.iconType == Icon.Type.FULL) {
				...
				{GlStateManager.color(1.0F, 1.0F, 1.0F, PASS_THREE_ALPHA);
		...
		if (absorb > 0) {
			for (int i = 9; i>=0; i--) {
				...
				if (i * 2 + 1 < absorb) {
					...
					GlStateManager.color(1.0F, 1.0F, 1.0F, PASS_SIX_ALPHA);
	...
	public void potionEffects(int x, int y, int k5, int i, int health){
		if (k5 == 52){
			if (i * 2 + 1 != health || health >=20){GlStateManager.color(1.0F, 1.0F, 1.0F, POTION_ALPHA);
